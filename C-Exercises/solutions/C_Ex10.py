# Sample Solution C-Exercise 10, WT 2022
...
# from C04 with added Expected Shortfall
...
if __name__ == "__main__":
    ...
    print('For alpha = ' + str(
        alpha[0] * 100) + '%: ' + 'Using the method of historic simulation to estimate the VaR we observe ' + str(
        np.sum(
            viol_hist[:,
            0])) + ' violations, while using the method of iid normally distributed log-returns we observe ' + str(
        np.sum(viol_lognormal[:, 0])) + ' violations. The expected number of violations is 10% x ' + str(
        len(loss) - td) + ' = ' + str((len(loss) - td) * 0.1) + '.')

    print('For alpha = ' + str(
        alpha[1] * 100) + '%: ' + 'Using the method of historic simulation to estimate the VaR we observe ' + str(
        np.sum(
            viol_hist[:,
            1])) + ' violations, while using the method of iid normally distributed log-returns we observe ' + str(
        np.sum(viol_lognormal[:, 1])) + ' violations. The expected number of violations is 5% x ' + str(
        len(loss) - td) + ' = ' + str((len(loss) - td) * 0.05) + '.')
